...
class MainActivity : AppCompatActivity() {
    ...
    private lateinit var listRepositories: RecyclerView
    ...
    private fun setupListeners() {
        btnConfirm.setOnClickListener{
            ...
            val dataNameUser = etNameUser.text.toString()
            saveUserLocal(dataNameUser)
    ...
    private fun saveUserLocal(name: String) {
        ...
        with(sharedPref.edit()){
            putString(getString(R.string.name_user), name)
            apply()
        }
    }
    ...
    private fun showUserName() {
        ...
        val name = sharedPref.getString(getString(R.string.name_user), "")

        if (name != ""){
            etNameUser.setText(name)
        }
    }
    ...
    fun getAllReposByUserName() {
        ...
        if (name != null && name != ""){
            val call = githubApi.getAllRepositoriesByUser(name)
            call.enqueue(object :Callback<List<Repository>>{
                override fun onResponse(call: Call<List<Repository>>, response: Response<List<Repository>>) {
                    if (response.isSuccessful){
                        ...
                        Toast.makeText(applicationContext, getString(R.string.msg_error), Toast.LENGTH_SHORT).show()
                    }
                }
